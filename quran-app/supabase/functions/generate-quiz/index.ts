...
// و ۵ سؤال تشریحی بسازد (جمعاً ۱۰ سؤال)، نتیجه را در ai_generated_quizzes ذخیره
// می‌کند و نسخه‌ای بدون پاسخ صحیح را برای فرانت‌اند برمی‌گرداند (تا تقلب ممکن نباشد).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const prompt = `شما یک دبیر قرآن در دورهٔ اول متوسطه هستید. بر اساس فعالیت‌های زیر از درس «${lesson.title}»، دقیقاً ۵ سؤال چندگزینه‌ای (هرکدام ۴ گزینه) و ۵ سؤال تشریحی به زبان فارسی طراحی کن (جمعاً ۱۰ سؤال). سؤالات باید فقط بر اساس همین متن باشند، نه اطلاعات بیرونی.
